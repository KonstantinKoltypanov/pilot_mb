...
import { ContentLayoutBody, ContentLayoutFooter } from "../DocumentCard/ContentLayoutHelpers";
...
export const DocumentHistory: React.FC = () => {
  const { id } = useParams();
  const {
    getDocumentHistoryApi: { fetch, data, loading },
  } = useDocumentResource();
  ...
  return (
    ...
    >
      ...
      <ContentLayoutFooter>
        {id && <Typography.Text>ID документа: {id}</Typography.Text>}
      </ContentLayoutFooter>
    </div>
  );
};
